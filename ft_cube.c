...
t_ray	ft_print_wall(t_ray ray, int x)
{
	...
	while (wall.y < ray.height)
	{
		if (wall.y < wall.startdraw)
			ft_pixel_put(&ray.img, x, wall.y, ft_trgb(0, ray.mapest->c_color[0], ray.mapest->c_color[1],ray.mapest->c_color[2]));
		else if (wall.y >= wall.startdraw && wall.y <= wall.enddraw)
		{
			...
			ft_pixel_put(&ray.img, x, wall.y, ft_trgb(0,ray.mapest->f_color[0], ray.mapest->f_color[1],ray.mapest->f_color[2]));
		wall.y++;
	}
	return (ray);
}
